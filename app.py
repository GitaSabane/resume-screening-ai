...
def compute_skill_match(resume_text, jd_text, skills):
    ...
    skills_in_jd = [s for s in skills if s.lower() in jd_lower]
...
def generate_suggestions(missing_skills):
    if not missing_skills:
        return ["Great job! Your resume covers all the skills mentioned in the job description."]
    suggestions = []
    suggestions.append(f"Consider adding these skills to your resume: {', '.join(missing_skills)}.")
    suggestions.append("Tailor your resume to include relevant keywords from the job description.")
    suggestions.append("Highlight your experience with the missing skills if you have them, using concrete examples.")
    return suggestions
...
# -------------------- Global after_request for all pages (except static) --------------------
@app.after_request
def add_cache_headers(response):
    # Skip static files (optional)
...
def register():
    if request.method == 'POST':
        ...
        if errors:
            return render_template('register.html', error='<br>'.join(errors), name=name, email=email)
...
def login():
    if request.method == 'POST':
        ...
        if errors:
            return render_template('login.html', error='<br>'.join(errors))
...
def upload_resume():
    ...
    try:
        ...
        if not resume_text:
            return jsonify({'error': 'Could not extract text from PDF. Ensure it is text-based.'}), 400

        name, email = extract_name_email(resume_text)
        resume_lower = clean_text(resume_text)
        detected_skills = [s for s in SKILLS if s.lower() in resume_lower]
...
def analyze():
    ...
    match_ratio, matched, missing, skills_in_jd = compute_skill_match(resume_text, jd_text, SKILLS)
    cos_sim = compute_cosine_similarity(resume_text, jd_text)
    final_score = round((0.6 * match_ratio + 0.4 * cos_sim) * 100, 2)

    skill_match_pct = round(match_ratio * 100, 2)
    content_relevance_pct = round(cos_sim * 100, 2)
...
def get_history():
    ...
    analyses = AnalysisHistory.query.filter_by(user_id=user_id).order_by(AnalysisHistory.created_at.desc()).limit(10).all()
    history_data = []
    for a in analyses:
        history_data.append({
            'id': a.id,
